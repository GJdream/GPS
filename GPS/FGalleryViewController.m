...
- (void)loadView
{
    ...
    if(SYSTEM_VERSION_LESS_THAN(@"7.0"))
    {
        ...
        _toolbar.tintColor                  = [UIColor colorWithHexString:@"A70014"];
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:@"Ver todas" style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] autorelease];
...
{
    if (!_hideTitle){
        [self setTitle:[NSString stringWithFormat:@"%i %@ %i", _currentIndex+1,@"de" , [_photoSource numberOfPhotosForPhotoGallery:self]]];
    }else{
        [self setTitle:@""];
    }
}
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [self arrangeThumbs];
    [self.navigationItem.rightBarButtonItem setTitle:@"Fechar"];
...
{
    _isThumbViewShowing = NO;
    [self.navigationItem.rightBarButtonItem setTitle:@"Ver todas"];
